...
//Submit Baseline Questionnaire
    $('#btnSubmitBaseline').on('click', function(event){
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                swal({title: "Done", text: result, type: "success"},
                function(){ 
    ...
    //Submit Baseline Post Study Questionnaire
    $('#btnSubmitPostStudy').on('click', function(event){
        event.preventDefault();
        ...
        //let authToken = 'Z3a6OQ+aLlcfHdMGSq10wLMvbKEIrAOuTRAU0YplEnY=';
        let url = urlDomain + 'insomnia/v1/patient/baseline180';
        $.ajax({
            url: url,
            type: 'POST',
            headers: {
    ...
    //Submit Baseline Post Study Questionnaire Final
    $('#btnSubmitPostStudyFinal').on('click', function(event){
        event.preventDefault();
        ...
        var optRateQty = getQuestionValues('myBaselineForm', 'optRateQty');
        var optAppUse = getQuestionValues('myBaselineForm', 'optAppUse');
        var optRedMed = getQuestionValues('myBaselineForm', 'optRedMed');
        var optNavApp = getQuestionValues('myBaselineForm', 'optNavApp');
        var optRecApp = getQuestionValues('myBaselineForm', 'optRecApp');
        var optExtApp = getQuestionValues('myBaselineForm', 'optExtApp');
        var optSatHelp = getQuestionValues('myBaselineForm', 'optSatHelp');

        var intervFeatures = getMultipleCheckedValues('myBaselineForm', 'interv');
        var intervOtherTxt = document.getElementById("featureOtherTxt").value;
        if(intervFeatures == 'Other'){
            intervFeatures = intervOtherTxt;
        }

        var optHealth = getQuestionValues('myBaselineForm', 'optHealth');
        var optSeekHp = getQuestionValues('myBaselineForm', 'optSeekHp');

        var any_comment = document.getElementById("commentTxt").value;
        var any_other_services = document.getElementById("otherServicesTxt").value;

        let authToken = window.localStorage.getItem("patientBaselineToken");
        let groupId = window.localStorage.getItem("patientGroupId");

        let url = urlDomain + '/insomnia/v1/patient/baselinepostintervention';

        if(intervFeatures == "" && groupId == "0"){
            swal({title: "", text: "Please let us know the features of the app you find most useful.", html: true});
        }else{
            $.ajax({
                url: url,
                type: 'POST',
                ...
                data: JSON.stringify({
                    ...
                    "apprating" : optRateQty,
                    "healthusefulnessofApp" : optAppUse,
                    "apphelpreduceMedications" : optRedMed,
                    "appnavigationexperience" : optNavApp,
                    "recommendApp" : optRecApp,
                    "appmetneed" : optExtApp,
                    "satisfaction" : optSatHelp,
                    "mostusefulfeature" : intervFeatures,
                    "generalsatisfaction" : optHealth,
                    "useagain" : optSeekHp,
                    "comments" : any_comment,
                    "anyotherhelp" : any_other_services
                ...
                error: function(msg){
                    ...
                    // console.log("Baseline Token: " + authToken);
                    if(msg.status == "201"){
                        swal({title: "Done", text: msg, type: "success"},
                            function(){ 
                                window.location.href = "baseline-completed.html";
                            }
                        );
